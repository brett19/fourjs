(function(FOUR) {
  ...
  var TextureLevel = FOUR.TextureLevel;
  ...
  DDS.load = function(texture, path, callback) {
    texture.unpackAlignment = 1;
    texture.name = path;

    FOUR.io.loadRaw(path, function(err, res) {
      if (err) {
        callback(err, null);
        return;
      }
      var rh = new FOUR.BinaryReader(res);
  ...
  DDSLoader.detect =  function(path) {
    var pathExt = path.substr(-3).toLowerCase();
    return pathExt === 'dds';
  };

  DDSLoader.load = function(tex, path, callback) {
    DDS.load(tex, path, callback);
  };

  TextureLoader.registerLoader(DDSLoader);
